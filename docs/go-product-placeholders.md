...
| `COMMUNITY_ASSET_SUBMISSIONS_ENABLED` | Enables member asset-pack drafts, review, publication, versioning, and protected downloads. | `false`; submission and download routes remain unavailable. | Enable only after administrators are ready to review rights, licenses, manifests, compatibility, previews, downloads, entitlements, and support status. |
...
Phase 6 completes the community asset-pack workflow without opening it by default. An active Community or Business benefit is required to create or edit a draft. Only one draft, submitted, changes-requested, or approved version may be pending for a published pack at a time, and the currently published version remains available until an administrator explicitly publishes its approved replacement.
...
A resource may be marked Legacy only after an administrator clears every review checklist item and records a reason. `APRIL_2025_RESOURCE_ID` remains empty and unused: Phase 6 performs no lookup, migration, fixture creation, production-data mutation, deployment, or launch. Keep `COMMUNITY_ASSET_SUBMISSIONS_ENABLED=false` until real review operations, content, authorization tests, and protected-link behavior have been verified in staging.
