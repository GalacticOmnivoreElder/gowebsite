...
Progress is stored at `omnivore_progress/{verified uid}` in the existing Firestore database, protected by the existing default-deny client rules. Only aggregate XP, guide badges, last route, bounded event IDs and rate counters are retained. Questions and user-supplied tags/names are neither sent nor stored. Unknown payload fields are rejected. Session replay excludes `/learn`, and the guide also has Clarity masking for navigation from an already instrumented page.
