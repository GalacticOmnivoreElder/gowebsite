...
import { deleteMentorPilotApplication, forwardRequestToRequestedMentor, getMentorshipPilotAdminDashboard, resolveMentorshipPilotReport, reviewMentorPilotApplication, reviewMentorshipPilotRequest, sendMentorSuggestions, updateMentorshipPilotEngagement } from "@/lib/mentorship-pilot-service";
...
export async function PATCH(request) {
  ...
  try {
    ...
    if (body.action === "review_mentor") return Response.json(await reviewMentorPilotApplication({ actor: gate.user, userId: String(body.userId || ""), decision: String(body.decision || ""), internalNotes: body.internalNotes, customerMessage: body.customerMessage }));
    if (body.action === "delete_mentor_application") return Response.json(await deleteMentorPilotApplication({ actor: gate.user, userId: String(body.userId || ""), reason: body.reason }));
