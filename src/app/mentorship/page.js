...
export const metadata = createMetadata({
  title: "GO Mentorship",
  description: "Explore verified Galactic Omnivore mentors and request structured mentorship.",
  path: "/mentorship",
});
...
export default function MentorshipPage() {
  ...
  const enabled = product.featureFlags.mentorDirectory && mentorship.featureFlags.mentorshipSystem && mentorship.featureFlags.publicMentorBrowsing;
    ...
    <main className="container mx-auto max-w-6xl px-4 py-12 md:py-16">
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div>
          ...
          <p className="mt-4 max-w-3xl text-lg text-muted-foreground">Browse mentors who have completed GO review and an interview. Choose someone with an available slot, then send a private mentorship request for GO to review and forward. Mentorship requests are open to eligible GO members from their private profile.</p>
        </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        ...
        <Card><CardContent className="p-5"><Users className="h-5 w-5 text-primary" /><h2 className="mt-3 font-semibold">The mentor decides</h2><p className="mt-1 text-sm text-muted-foreground">The selected mentor reviews the shared request and may accept or decline.</p></CardContent></Card>
      </div>

      {enabled ? <MentorDirectory /> : (
        <Card className="mt-10 border-primary/30"><CardContent className="p-8"><h2 className="text-xl font-semibold">Official mentor directory</h2><p className="mt-2 text-muted-foreground">Mentor profiles are currently unavailable. Please try again later.</p></CardContent></Card>
      )}
    </main>
  );
}
