...
export function MentorshipAdminWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    if (!response.ok) throw new Error(result.error || "Mentorship operations could not be loaded.");
    setData(result);
  }, [call]);
  ...
  if (!data) return <p className="py-10 text-center text-muted-foreground">Loading mentorship operations…</p>;
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="text-sm font-semibold uppercase text-primary">Mentorship operations</p>
