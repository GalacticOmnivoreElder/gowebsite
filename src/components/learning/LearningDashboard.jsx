...
export function LearningDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5" />Courses and workshops</CardTitle></CardHeader><CardContent className="space-y-3">{data?.enrollments?.map((enrollment) => <div key={enrollment.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border p-4"><div><p className="font-semibold">{enrollment.itemTitle}</p><Badge className="mt-2 capitalize">{enrollment.state.replaceAll("_", " ")}</Badge>{enrollment.courseId && <p className="mt-2 text-sm text-muted-foreground">{enrollment.attendanceMode === "online" ? "Online" : "GOHQ"} · {enrollment.completedDays?.length || 0}/5 days complete</p>}{enrollment.badge && <p className="mt-2 font-semibold text-primary">{enrollment.badge.title}</p>}{enrollment.courseId && <p className="mt-2 text-sm text-muted-foreground">{enrollment.attendanceMode === "online" ? "Online" : "GOHQ"} · {enrollment.completedDays?.length || 0}/5 days complete</p>}{enrollment.badge && <p className="mt-2 font-semibold text-primary">{enrollment.badge.title}</p>}</div><Button asChild variant="outline"><Link href={`/education/${enrollment.itemSlug}`}>Open</Link></Button></div>)}{data?.enrollments?.length === 0 && <p className="py-6 text-center text-muted-foreground">No course or workshop enrollments yet.</p>}</CardContent></Card>
