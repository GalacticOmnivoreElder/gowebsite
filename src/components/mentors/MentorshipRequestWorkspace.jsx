...
const initialForm = {
  ...
  preferredTimeframe: "two_to_four_weeks", timeframeDetails: "", languagePreferences: "English", timeZone: "Europe/Skopje",
...
export function MentorshipRequestWorkspace({ requestedMentorId = "" }) {
  ...
  const request = async (mode) => {
    ...
    try {
      ...
      const response = await fetch("/api/mentorship/requests", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...form, requestId, mode, requestedMentorId: requestedMentorId || null, languagePreferences: csv(form.languagePreferences), projectLinks: csv(form.projectLinks) }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error === "mentorship_membership_required" ? "An active GO membership is required to request mentorship." : result.error || "Mentorship request could not be saved.");
      if (mode !== "draft") {
  ...
  return <div className="space-y-6">
    {requestedMentorId ? <Card className="border-primary/30 bg-primary/5"><CardContent className="p-5">{selectedMentor?.loading ? <p className="text-sm text-muted-foreground">Loading selected mentor…</p> : selectedMentor?.error ? <div className="flex flex-wrap items-center justify-between gap-3"><p role="alert" className="text-sm text-destructive">{selectedMentor.error}</p><Button asChild variant="outline" size="sm"><Link href="/mentorship">Choose another mentor</Link></Button></div> : <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase text-primary">Applying to</p><p className="mt-1 text-lg font-semibold">{selectedMentor?.displayName}</p>{selectedMentor?.professionalHeadline ? <p className="text-sm text-muted-foreground">{selectedMentor.professionalHeadline}</p> : null}<p className="mt-2 text-sm">{selectedMentor?.availableSlots || 0} available slot{selectedMentor?.availableSlots === 1 ? "" : "s"}</p></div><Button asChild variant="outline" size="sm"><Link href="/mentorship">Choose another mentor</Link></Button></div>}</CardContent></Card> : null}
