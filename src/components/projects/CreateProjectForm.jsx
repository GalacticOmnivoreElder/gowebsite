...
const CreateProjectContent = observer(() => {
  ...
  const projectTypeOptions =
    ...
    !PROJECT_TYPES.some(
      (projectType) =>
        projectType.toLowerCase() === watchedValues.type.trim().toLowerCase()
    )
      ? [...PROJECT_TYPES, watchedValues.type]
      : PROJECT_TYPES;
  ...
  useEffect(() => {
    ...
    const verifyCreatorAccess = async () => {
      ...
      const canCreateCreatorProject =
        result?.permissions?.canCreateCreatorProjects ??
        MobxStore.permissions?.permissions?.canCreateCreatorProjects ??
        MobxStore.user?.activeMember === true;

      if (canCreateCreatorProject === false) {
        router.replace(CREATOR_MEMBERSHIP_URL);
        return;
      }

      if (canCreateCreatorProject !== true) {
        setAccessCheckError(
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      const projectData = {
        ...formData,
        intent: searchParams.get('intent') === 'hire-talent' ? 'hire-talent' : 'creator',
            ...
            render={({ field }) => (
              <>
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  >
                    <Plus className="h-4 w-4" />
                    <span className="sr-only">Use custom project type</span>
                  </Button>
                </div>
