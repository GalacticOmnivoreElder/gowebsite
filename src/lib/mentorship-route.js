...
import { authorizeMentorshipAction } from "@/lib/mentorship";

export async function requireMentorshipUser(request, action) {
...
export async function requireMentorshipAdmin(request) {
  const user = await getRequestUser(request);
  ...
  return { user, config: getMentorshipConfig() };
}
