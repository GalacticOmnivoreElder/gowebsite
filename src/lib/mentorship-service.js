...
import {
  ...
  MENTORSHIP_ENGAGEMENT_STATUSES,
...
async function getAvailablePublicMentor(mentorId, db = adminDb) {
  ...
  if (
    !userDoc.exists ||
    !profileDoc.exists ||
    !hasMentorToolAccess(userDoc.data()) ||
...
function lockRef(db, userId) {
  return db.collection("mentorship_active_requests").doc(userId);
}
...
export async function saveMentorApplication({ user, input = {}, action = "save", db = adminDb, now = new Date() }) {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    const clean = cleanMentorApplicationProfile({ ...previous, ...input }, { submitting });
  ...
  return { profile: serializeMentorProfile(user.uid, result.profile, { admin: false }), status: result.profile.status };
}

export async function reviewMentorApplication({ actor, userId, decision, internalNotes = "", customerMessage = "", db = adminDb, now = new Date() }) {
...
export async function deleteMentorApplication({ actor, userId, reason = "", db = adminDb, now = new Date() }) {
  if (!actor?.admin) throw workflowError("Platform admin access required", "staff_required", 403);
  ...
  const [relatedApplications, relatedRequests, relatedEngagements, learningItems, assetPacks] = await Promise.all([
  ...
  if (!relatedApplications.empty || !relatedRequests.empty || !relatedEngagements.empty || hasPublishedLearning || hasPublishedAssets) {
    throw workflowError(
...
export async function saveMentorshipRequest({ user, requestId = "", input = {}, mode = "submit", db = adminDb, now = new Date() }) {
  const config = getMentorshipConfig();
  actorAllowed(user, config, "create_request");
  const requestInput = /** @type {Record<string, any>} */ (input);
  if (!config.under18MentorshipEnabled && requestInput.isAdult !== true) throw workflowError("You must be 18 or older to request mentorship", "adult_confirmation_required", 403);
  const submitting = mode === "submit";
  const requestedMentor = requestInput.requestedMentorId
    ? await getAvailablePublicMentor(requestInput.requestedMentorId, db)
    : null;
  const requestRef = requestId ? db.collection("mentorship_requests").doc(requestId) : db.collection("mentorship_requests").doc(stableId("request", user.uid, crypto.randomUUID()));
  const result = await db.runTransaction(async (transaction) => {
    ...
    const clean = cleanMentorshipRequest({ ...previous, ...requestInput }, { submitting });
  ...
  return serializeMentorshipRequest(result.id, result.data);
}

export async function updateMentorshipRequest({ user, requestId, action, message = "", db = adminDb, now = new Date() }) {
...
export async function reviewMentorshipRequest({ actor, requestId, decision, customerMessage = "", internalNotes = "", db = adminDb, now = new Date() }) {
  if (!actor?.admin) throw workflowError("Platform admin access required", "staff_required", 403);
...
export async function sendMentorSuggestions({ actor, requestId, suggestions = [], db = adminDb, now = new Date() }) {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    for (const [index, item] of mentorDocs.entries()) {
      ...
      const accepting = hasMentorToolAccess(user) && (profile.publicProfileConsent === true || user.mentorPublicProfileEnabled === true) && getMentorCapacity(profile).accepting;
...
export async function forwardRequestToRequestedMentor({ actor, requestId, customerMessage = "", db = adminDb, now = new Date() }) {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    if (
      ...
      !hasMentorToolAccess(mentorUserDoc.data()) ||
    ...
    const mentorProfile = serializeMentorProfile(mentorId, profile);
    const suggestion = {
      id: suggestionId,
      requestId,
      mentorId,
      ...
      shownAt: now,
      selectedAt: now,
      expiresAt: addDays(now, getMentorshipConfig().applicationResponseTargetWorkingDays),
...
export async function getMentorshipSuggestionsForUser({ user, db = adminDb }) {
  const requests = await db.collection("mentorship_requests").where("menteeUserId", "==", user.uid).limit(100).get();
...
export async function declineMentorSuggestion({ user, suggestionId, db = adminDb, now = new Date() }) {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    const requestRef = db.collection("mentorship_requests").doc(suggestion.requestId);
    const requestDoc = await transaction.get(requestRef);
...
export async function applyToMentorSuggestion({ user, suggestionId, message = "", dataSharingConsent = false, db = adminDb, now = new Date() }) {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    if (!mentorDoc.exists || !hasMentorToolAccess(mentorDoc.data())) throw workflowError("This mentor is no longer available", "mentor_unavailable", 409);
  ...
  return serializeMentorshipApplication(result.id, result.data);
}

export async function respondToMentorApplication({ user, applicationId, action, response = "", db = adminDb, now = new Date() }) {
  if (!user?.admin && !hasMentorToolAccess(user?.userData || {}, { now })) {
    throw workflowError("Active verified Mentor membership is required", "mentor_membership_required", 403);
  }
  const applicationRef = db.collection("mentorship_applications").doc(applicationId);
  const result = await db.runTransaction(async (transaction) => {
    ...
    if (action === "withdraw") {
      ...
      transaction.update(db.collection("mentorship_requests").doc(application.requestId), { status: "suggestions_sent", updatedAt: now });
    ...
    const requestRef = db.collection("mentorship_requests").doc(application.requestId);
    const requestDoc = await transaction.get(requestRef);
    if (!requestDoc.exists) throw workflowError("Mentorship request not found", "not_found", 404);
    ...
    transaction.update(profileRef, { activeEngagementCount: Math.max(0, Number(profileDoc.data().activeEngagementCount) || 0) + 1, updatedAt: now });
...
export async function updateMentorshipEngagement({ user, engagementId, action, payload = {}, db = adminDb, now = new Date() }) {
  const actionPayload = /** @type {Record<string, any>} */ (payload);
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    const current = doc.data();
    if (!user?.admin && user?.uid === current.mentorId && !hasMentorToolAccess(user?.userData || {}, { now })) {
      throw workflowError("Active verified Mentor membership is required", "mentor_membership_required", 403);
    }
    if (!MENTORSHIP_ENGAGEMENT_STATUSES.includes(current.status)) throw workflowError("This engagement is no longer available", "invalid_status", 409);
    if (action === "confirm_agreement") {
      const agreement = cleanMentorshipAgreement(actionPayload.agreement || actionPayload);
      ...
      transaction.update(ref, { agreement, agreementVersion: version, agreementConfirmations: confirmations, status: active ? "active" : "ready_to_start", startDate: agreement.startDate, targetEndDate: agreement.targetEndDate, nextCheckInDate: active ? addDays(now, getMentorshipConfig().checkInFrequencyDays) : null, activatedAt: active ? now : null, updatedAt: now });
    ...
    if (action === "submit_checkin") {
      ...
      const checkIn = cleanCheckIn(actionPayload);
      const checkInId = stableId("checkin", engagementId, user.uid, String(actionPayload.idempotencyKey || new Date(now).toISOString().slice(0, 10)));
      const checkInRef = db.collection("mentorship_checkins").doc(checkInId);
      const existing = await transaction.get(checkInRef);
      if (existing.exists) return { recipientUserId: user.uid === current.mentorId ? current.menteeUserId : current.mentorId, status: "checkin_saved", checkIn: serializeMentorshipCheckIn(checkInId, existing.data()), idempotent: true };
      const data = { ...checkIn, id: checkInId, engagementId, submittedBy: user.uid, submittedAt: now };
      transaction.create(checkInRef, data);
      transaction.update(ref, { lastCheckInDate: now, nextCheckInDate: addDays(now, getMentorshipConfig().checkInFrequencyDays), updatedAt: now });
      ...
      return { recipientUserId: user.uid === current.mentorId ? current.menteeUserId : current.mentorId, status: "checkin_saved", checkIn: serializeMentorshipCheckIn(checkInId, data), idempotent: false };
    }
    ...
    if (action === "end_early") {
      const reason = ["goal_changed", "scheduling_conflict", "mentor_unavailable", "mentee_unavailable", "fit_not_appropriate", "expectations_not_aligned", "boundary_or_conduct_concern", "ended_by_go", "other"].includes(actionPayload.reasonCategory) ? actionPayload.reasonCategory : "other";
      await releaseMentorCapacity(transaction, db, current, now);
...
export async function submitMentorshipClosingFeedback({ user, engagementId, input, db = adminDb, now = new Date() }) {
  const engagement = await db.collection("mentorship_engagements").doc(engagementId).get();
...
export async function resolveMentorshipReport({ actor, reportId, status = "resolved", notes = "", db = adminDb, now = new Date() }) {
  if (!actor?.admin) throw workflowError("Platform admin access required", "staff_required", 403);
...
export async function getMentorshipDashboard(user, { db = adminDb } = {}) {
  ...
  const [requests, menteeApplications, mentorApplications, menteeEngagements, mentorEngagements, mentorProfile, mentorApplication] = await Promise.all([
    db.collection("mentorship_requests").where("menteeUserId", "==", user.uid).limit(100).get(),
    ...
    db.collection("mentorship_engagements").where("menteeUserId", "==", user.uid).limit(100).get(),
  ...
  const requestDtos = requests.docs.map((doc) => serializeMentorshipRequest(doc.id, doc.data()));
  const suggestions = (await Promise.all(requests.docs.map((doc) => db.collection("mentorship_suggestions").where("requestId", "==", doc.id).limit(20).get()))).flatMap((snapshots) => snapshots.docs.map((doc) => serializeSuggestion(doc.id, doc.data())));
  ...
  const checkIns = (await Promise.all([...engagementDocs.keys()].map((id) => db.collection("mentorship_checkins").where("engagementId", "==", id).limit(20).get()))).flatMap((snapshots) => snapshots.docs.map((doc) => serializeMentorshipCheckIn(doc.id, doc.data())));
  return { mentorProfile: mentorProfile.exists ? serializeMentorProfile(user.uid, mentorProfile.data()) : null, mentorApplication: mentorApplication.exists ? serializeMentorApplicationSummary(mentorApplication.id, mentorApplication.data(), mentorProfile.exists ? mentorProfile.data() : {}) : mentorProfile.exists ? serializeMentorApplicationSummary(user.uid, {}, mentorProfile.data()) : null, requests: requestDtos, suggestions, applications: [...appDocs.values()].map((doc) => serializeMentorshipApplication(doc.id, doc.data())), engagements: [...engagementDocs.values()].map((doc) => serializeMentorshipEngagement(doc.id, doc.data())), checkIns };
}

export async function getMentorshipAdminDashboard({ actor, db = adminDb }) {
  if (!actor?.admin) throw workflowError("Platform admin access required", "staff_required", 403);
  ...
  const mentorRows = mentorApplications.docs.map((doc, index) => ({ id: doc.id, application: serializeMentorApplicationSummary(doc.id, doc.data(), profiles[index].exists ? profiles[index].data() : {}), profile: profiles[index].exists ? serializeMentorProfile(doc.id, profiles[index].data(), { admin: true }) : null }));
  const config = getMentorshipConfig();
  ...
  return { config, counts, mentorApplications: mentorRows, requests: requests.docs.map((doc) => serializeMentorshipRequest(doc.id, doc.data(), { includeInternal: true })), suggestions: suggestions.docs.map((doc) => ({ ...serializeSuggestion(doc.id, doc.data()), privateStaffNotes: doc.data().privateStaffNotes || "" })), applications: applications.docs.map((doc) => serializeMentorshipApplication(doc.id, doc.data(), { includePrivate: true })), engagements: engagements.docs.map((doc) => serializeMentorshipEngagement(doc.id, doc.data(), { includePrivate: true })), reports: reports.docs.map((doc) => ({ id: doc.id, engagementId: doc.data().engagementId, category: doc.data().category, details: doc.data().details, status: doc.data().status, createdAt: asDate(doc.data().createdAt)?.toISOString() || null })), alerts: alerts.docs.map((doc) => ({ id: doc.id, ...doc.data(), createdAt: asDate(doc.data().createdAt)?.toISOString() || null })), audit: auditEvents.docs.map((doc) => ({ id: doc.id, ...doc.data(), createdAt: asDate(doc.data().createdAt)?.toISOString() || null })).sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || "")) };
}

export const __mentorshipInternals = Object.freeze({ actorAllowed, releaseRequestLock });
