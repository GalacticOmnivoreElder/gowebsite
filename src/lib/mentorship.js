...
export const MENTORSHIP_APPLICATION_STATUSES = Object.freeze([
  "pending", "accepted", "declined", "withdrawn", "expired", "cancelled_by_go",
]);
export const MENTORSHIP_ENGAGEMENT_STATUSES = Object.freeze([
  "awaiting_agreement", "ready_to_start", "active", "paused", "completion_pending", "completed", "ended_early", "cancelled", "under_review",
]);

export const MENTORSHIP_CONSENT_VERSION = "mentorship-v2";
export const MENTOR_TERMS_VERSION = "mentor-terms-v2";
...
export function cleanMentorApplicationProfile(input = {}, { submitting = false } = {}) {
...
export function cleanMentorshipRequest(input = {}, { submitting = false } = {}) {
...
export function authorizeMentorshipAction(user, action, config) {
  ...
  if (action === "apply_to_suggestion" && (!config.featureFlags.mentorshipRequests || !hasCommunityContentAccess(user.userData || {}, { admin: user.admin }))) return { allowed: false, reason: "mentorship_membership_required" };
  if (action === "manage_active_mentorship" && !hasCommunityContentAccess(user.userData || {}, { admin: user.admin })) return { allowed: false, reason: "mentorship_membership_required" };
  return { allowed: true };
}
...
export function serializeMentorProfile(id, data = {}, { admin = false } = {}) {
  const publicFields = {
    id,
...
export function serializeMentorshipRequest(id, data = {}, { includeInternal = false } = {}) {
  const value = {
    id,
...
export function serializeMentorshipApplication(id, data = {}, { includePrivate = false } = {}) {
...
export function serializeMentorshipEngagement(id, data = {}, { includePrivate = false } = {}) {
...
export function serializeMentorshipCheckIn(id, data = {}) {
