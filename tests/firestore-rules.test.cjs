...
beforeEach(async () => {
  ...
  await testEnv.withSecurityRulesDisabled(async (context) => {
    ...
    await Promise.all([
      ...
      setDoc(doc(db, "learning_courses", "course-1"), { status: "published" }),
      setDoc(doc(db, "learning_course_interest", "interest-1"), { userId: "member" }),
...
const serverOnlyDocuments = [
  ...
  ["learning_courses", "course-1"],
  ["learning_course_interest", "interest-1"],
